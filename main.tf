...
resource "aws_sqs_queue" "queue_with_kms" {
  count = length(var.kms_alias) != 0 || length(var.existing_kms_key) != 0 && length(var.redrive_arn) == 0 && length(var.policy) != 0 ? 1 : 0
  name  = var.name
  ...
  fifo_queue                        = var.fifo_queue
  content_based_deduplication       = var.content_based_deduplication
  kms_master_key_id                 = length(var.kms_alias) != 0 ? aws_kms_key.sqs_kms_key[0].key_id: var.existing_kms_key
  kms_data_key_reuse_period_seconds = 300
...
resource "aws_iam_user" "sqs_with_kms_iam_user" {
  count = length(var.policy) != 0 && length(var.kms_alias) != 0 || length(var.existing_kms_key) != 0 ? var.number_of_users : 0
...
resource "aws_iam_user_policy" "sqs_with_kms_user_policy" {
  count = length(var.kms_alias) != 0 || length(var.existing_kms_key) != 0 && length(var.redrive_arn) == 0 && length(var.policy) != 0 ? var.number_of_users : 0

  name   = "${var.iam_user_policy_name}SQSPolicy"
  user   = element(aws_iam_user.sqs_with_kms_iam_user.*.name, count.index)
  policy = var.kms_alias != 0 ? data.aws_iam_policy_document.sqs_with_kms_policy_document[0].json: data.aws_iam_policy_document.sqs_with_existing_kms_policy_document[0].json
}
...
data "aws_iam_policy_document" "sqs_with_existing_kms_policy_document" {
  count     = length(var.existing_kms_key) != 0 && length(var.redrive_arn) == 0 && length(var.policy) != 0 ? 1 : 0
  policy_id = "${var.sqs_iam_user}SQSPolicy"
  ...
  statement {
    ...
    resources = [
      aws_kms_key.sqs_kms_key[0].arn,
      "arn:aws:kms:${data.aws_region.current.name}:${data.aws_caller_identity.current.account_id}:key/${var.existing_kms_key}",
    ]
